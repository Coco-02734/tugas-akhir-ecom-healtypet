...
class Auth extends CI_Controller
{
    ...
    {
        $data['title'] = 'Halaman Login | Dokter Healty Pets';
        $this->load->view('dokter/templates/auth_header', $data);
        $this->load->view('dokter/login', $data);
        $this->load->view('dokter/templates/auth_footer');
    }
        ...
        if ($user) {
            ...
            if ($user['is_active'] == 1) {
                ...
                if (password_verify($password, $user['password'])) {
                    ...
                    if ($user['role_id'] == 3) {
                        if ($bio['is_verify'] == 0) {
                            redirect('dokter/pengajuan');
                        } else if ($bio['is_verify'] == 3) {
    ...
    {
        $data['title'] = 'Halaman Registrasi | Dokter Healty Pets';
        $this->load->view('dokter/templates/auth_header', $data);
        $this->load->view('dokter/register', $data);
        $this->load->view('dokter/templates/auth_footer');
    }
    ...
    public function registration()
    {
        ...
        $this->db->insert('biodata_dokter', $triggerBio);
    ...
    private function _sendEmail($token, $type)
    {
        ...
        if ($type == 'verify') {
            ...
            // Subject email
            $this->email->subject('Verifikasi Akun Dokter Healty Pets ( No-Reply )');

            // Isi email
            $this->email->message('Silahkan klik <a href=' . base_url() . 'dokter/auth/verify?email=' . $this->input->post('email') .
                '&token=' . urlencode($token) . '> Disini </a> untuk aktifkan akun');
        } else if ($type == 'forgot') {
            // Subject email
            $this->email->subject('Reset Password Akun ( No-Reply )');
            // Isi email
            $this->email->message('Silahkan klik <a href=' . base_url() . 'dokter/resetpassword?email=' . $this->input->post('email') .
